...
<form method="post" action="voucher_action_modifier.php?id=<?php if(isset($_GET['id'])) echo $_GET['id'];?>">
<div class="imprimer">
  <input name="Enregistrer" type="submit"  value="Enregistrer" />
</div>
<input type="hidden" name="num_voucher" value="<?php 
	...
	<div class="date">
       <h3> Monastir le  <input type="hidden" name="date" value="<?php if(isset($_GET['id'])) echo $r['date'];else {$da=date("Y-m-d"); echo $da; } ?>"  />
		...
		<tr>
            ...
            <h1>Voucher N&ordm;: ( <?php if (isset($_GET['id']))echo $_GET['id'] ;else echo ""; ?> )
		...
		<tr><td><strong>NIGHTS:</strong></td><td  colspan="3"><input type="text" name="nuit" value="<?php if (isset($_GET['id']))echo $r['nuit'] ;else echo ""; ?>"  /></td></tr>
		<td><strong>CHECK IN:</strong></td><td><input type="date" name="checkin" value="<?php if (isset($_GET['id']))echo $r['checkin'] ;else echo ""; ?>"   /></td>
		<td><strong>CHECK OUT:</strong></td><td><input type="date" name="checkout" value="<?php if (isset($_GET['id']))echo $r['checkout'] ;else echo ""; ?>"   /></td></tr>
		<tr><td colspan="4"><strong>Accommodation Detail:</strong></td></tr>
		<tr><td colspan="4"><textarea name="accomm"><?php if (isset($_GET['id']))echo $r['accomm'] ;else echo ""; ?></textarea></td></tr>
