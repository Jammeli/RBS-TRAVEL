...
if(isset($_GET['id_voyage'])){
	...
	if($_GET['id_voyage']=="") header("location: recherche_voyage.php");
...
if(isset($_GET['idsupp'])){
   ...
   $sqldelete=mysql_query("select image from images_voyage where id='".$_GET['idsupp']."'");
   $resdelete=mysql_fetch_row($sqldelete);
   unlink($resdelete[0]);

	mysql_query("delete from images_voyage where id_voyage='".$_GET['id_voyage']."' and id='".$_GET['idsupp']."'") or die(mysql_error());
	
			header("location: images_voyage.php?id_voyage=".$_GET['id_voyage']);

	}

if(isset($_GET['edit_profil'])){
	mysql_query("update images_voyage set type='simple' where id_voyage='".$_GET['id_voyage']."' and type='profil'") or die(mysql_error());

	mysql_query("update images_voyage set type='profil' where  id='".$_GET['edit_profil']."'and id_voyage='".$_GET['id_voyage']."'");
	header("location: images_voyage.php?id_voyage=".$_GET['id_voyage']);
	}

if(isset($_GET['edit_simple'])){
	mysql_query("update images_voyage set type='simple' where  id='".$_GET['edit_simple']."'and id_voyage='".$_GET['id_voyage']."'");
		header("location: images_voyage.php?id_voyage=".$_GET['id_voyage']);
...
<body class="fixed-top">
 ...
 elseif($upload1=="erreur"){echo '<span class="label label-important">NOTE!</span>
	...
	mysql_query("update images_voyage set type='simple' where id_voyage='".$_GET['id_voyage']."' and type='profil'") or die(mysql_error());
	 ...
	 mysql_query("INSERT INTO `images_voyage` (`id`, `id_voyage`, `type`, `image`) VALUES (NULL, '".$_GET['id_voyage']."', '".$_POST['type']."', '".$upload1."');");
                ...
                <div class="span12">
                    ...
                    <div class="widget red">
                        ...
                        <div class="widget-body form">
                           ...
                           <?php
						   	$sqlall=mysql_query("select * from images_voyage where id_voyage='".$_GET['id_voyage']."'") or die(mysql_error());
                                ...
                                <a id="supp<?php echo $reall['id'];?>"  
                                   ...
                                   onMouseOver="document.getElementById('supp<?php echo $reall['id'];?>').style.display='';"
                                style="position:absolute; margin-left:-25px; display:none; background-color:#FFF; height:20px; width:20px; margin-top:5px; text-align:center; text-decoration:none;" href="javascript: if(confirm('Voulez vous effacer l\'image?')) document .location.href='images_voyage.php?id_voyage=<?php echo $_GET['id_voyage'];?>&idsupp=<?php echo $reall['id'];?>'; " ><i class="icon-remove"   ></i></a>
...
<?php if( $reall['id']!="profil") {?><li><a href="<?php echo $_SERVER['PHP_SELF'];?>?id_voyage=<?php echo $_GET['id_voyage'];?>&edit_profil=<?php echo $reall['id'];?>">Image de profil</a></li> <?php }?>
<?php if( $reall['id']!="simple") {?><li><a href="<?php echo $_SERVER['PHP_SELF'];?>?id_voyage=<?php echo $_GET['id_voyage'];?>&edit_simple=<?php echo $reall['id'];?>">Image Simple</a></li> <?php }?>
