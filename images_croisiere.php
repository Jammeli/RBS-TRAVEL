...
if(isset($_GET['id_croisiere'])){
	
	if($_GET['id_croisiere']=="") header("location: recherche_croisiere.php");
...
if(isset($_GET['idsupp'])){
   ...
   $sqldelete=mysql_query("select image from images_croisiere where id='".$_GET['idsupp']."'");
   $resdelete=mysql_fetch_row($sqldelete);
   unlink($resdelete[0]);

	mysql_query("delete from images_croisiere where id_croisiere='".$_GET['id_croisiere']."' and id='".$_GET['idsupp']."'") or die(mysql_error());
	
			header("location: images_croisiere.php?id_croisiere=".$_GET['id_croisiere']);

	}

if(isset($_GET['edit_profil'])){
	mysql_query("update images_croisiere set type='simple' where id_croisiere='".$_GET['id_croisiere']."' and type='profil'") or die(mysql_error());

	mysql_query("update images_croisiere set type='profil' where  id='".$_GET['edit_profil']."'and id_croisiere='".$_GET['id_croisiere']."'");
	header("location: images_croisiere.php?id_croisiere=".$_GET['id_croisiere']);
	}

if(isset($_GET['edit_simple'])){
	mysql_query("update images_croisiere set type='simple' where  id='".$_GET['edit_simple']."'and id_croisiere='".$_GET['id_croisiere']."'");
		header("location: images_croisiere.php?id_croisiere=".$_GET['id_croisiere']);
...
<body class="fixed-top">
 ...
 elseif($upload1=="erreur"){echo '<span class="label label-important">NOTE!</span>
	...
	mysql_query("update images_croisiere set type='simple' where id_croisiere='".$_GET['id_croisiere']."' and type='profil'") or die(mysql_error());
	 ...
	 mysql_query("INSERT INTO `images_croisiere` (`id`, `id_croisiere`, `type`, `image`) VALUES (NULL, '".$_GET['id_croisiere']."', '".$_POST['type']."', '".$upload1."');");
                ...
                <div class="span12">
                    ...
                    <div class="widget red">
                        ...
                        <div class="widget-body form">
                           ...
                           <?php
						   	$sqlall=mysql_query("select * from images_croisiere where id_croisiere='".$_GET['id_croisiere']."'") or die(mysql_error());
                                ...
                                <a id="supp<?php echo $reall['id'];?>"  
                                   ...
                                   onMouseOver="document.getElementById('supp<?php echo $reall['id'];?>').style.display='';"
                                style="position:absolute; margin-left:-25px; display:none; background-color:#FFF; height:20px; width:20px; margin-top:5px; text-align:center; text-decoration:none;" href="javascript: if(confirm('Voulez vous effacer l\'image?')) document .location.href='images_croisiere.php?id_croisiere=<?php echo $_GET['id_croisiere'];?>&idsupp=<?php echo $reall['id'];?>'; " ><i class="icon-remove"   ></i></a>
...
<?php if( $reall['id']!="profil") {?><li><a href="<?php echo $_SERVER['PHP_SELF'];?>?id_croisiere=<?php echo $_GET['id_croisiere'];?>&edit_profil=<?php echo $reall['id'];?>">Image de profil</a></li> <?php }?>
<?php if( $reall['id']!="simple") {?><li><a href="<?php echo $_SERVER['PHP_SELF'];?>?id_croisiere=<?php echo $_GET['id_croisiere'];?>&edit_simple=<?php echo $reall['id'];?>">Image Simple</a></li> <?php }?>
